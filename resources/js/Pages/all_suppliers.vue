            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div v-else class="overflow-x-auto rounded-lg">
                    <table class="w-full">
                        ...
                        <tbody>
                        <tr v-for="(item,index) in filteredSuppliers" :key="item.id" class="border-b hover:bg-gray-50">
...
// import EditSupplierPopup from "@/Components/EditSupplierPopup.vue";
...
const goToRestorePage = () => {
    ...
    router.visit('/suppliers/restore')
}

const goToAddPage = () => {
    localStorage.setItem('supplierReferrer', '/suppliers/all')
    ...
    if (locationFilter.value === 'all') {
        ...
        filteredSuppliers.value = suppliers.value.filter(supplier => {
            return jabodetabekCities.some(city =>
                supplier.city && supplier.city.toLowerCase().includes(city)
            )
        })
...
const deleteSupplier = async (id, supplierName) => {
    const result = await Swal.fire({
        ...
        html: `Are you sure you want to delete <strong>${supplierName}</strong>?<br>The supplier will be moved to the restore page.`,
        icon: 'warning',
    ...
    if(result.isConfirmed) {
        try {
            ...
            Swal.fire({
                title: 'Error',
                text: error.response?.data?.errors?.message || 'Failed to delete supplier.',
                icon: 'error',
                confirmButtonText: 'OK',
                customClass: {
                    confirmButton: 'bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-md focus:outline-none transition-colors duration-200'
...
const fetchAllSuppliers = async () => {
    try {
        ...
        Swal.fire({
            title: 'Error',
            text: 'Failed to fetch all suppliers for export.',
            icon: 'error',
            confirmButtonText: 'OK'
        })
        return []
...
const exportToPDF = async () => {
    ...
    let suppliersToExport = allSuppliers
    if (locationFilter.value === 'jabodetabek') {
        suppliersToExport = allSuppliers.filter(supplier => {
            return jabodetabekCities.some(city =>
                supplier.city && supplier.city.toLowerCase().includes(city)
            )
        })
    ...
    suppliersToExport.forEach((item, index) => {
        const rowData = [
            index + 1,
            item.name,
            item.company_name,
...
const exportToExcel = async () => {
    ...
    let fileName = 'supplier-list'
    if (locationFilter.value === 'jabodetabek') {
        fileName = 'jabodetabek-suppliers'
    } else if (locationFilter.value === 'non-jabodetabek') {
        fileName = 'non-jabodetabek-suppliers'
    }
