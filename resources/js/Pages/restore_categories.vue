        ...
        <Sidebar :auth="auth">
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div v-else-if="categories.length === 0" class="text-center py-10">
                    ...
                    <h3 class="text-lg font-medium text-gray-700 mb-2">No Deleted Categories</h3>
                    <p class="text-gray-500">There are no deleted categories to restore.</p>
                </div>
...
const saveFilterState = () => {
    ...
    localStorage.setItem('restoreCategoryFilterState', JSON.stringify(filterState))
}

const restoreFilterState = () => {
    const savedState = localStorage.getItem('restoreCategoryFilterState')
    if (savedState) {
        try {
...
const goToAllCategories = () => {
    router.visit('/categories/all')
}

const goToDetail = (id) => {
    saveFilterState()
    localStorage.setItem('categoryReferrer', '/categories/restore')
