        ...
        <Sidebar :auth="auth">
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex gap-3 items-center">
                        ...
                        >
                            ...
                            <p class="text-lp-green font-medium">Back to Suppliers</p>
                        </button>
                ...
                <div v-else-if="suppliers.length === 0" class="text-center py-10">
                    ...
                    <h3 class="text-lg font-medium text-gray-700 mb-2">No Deleted Suppliers</h3>
                    <p class="text-gray-500">There are no deleted suppliers to restore.</p>
                </div>
                ...
                <div v-else class="overflow-x-auto rounded-lg">
                    <table class="w-full">
                        ...
                        <tbody>
                        <tr v-for="(item,index) in suppliers" :key="item.id" class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-8 py-3 text-right">
                                <div class="flex justify-end gap-1">
                                    <!-- Button Delete -->
                                    <button
                                        @click="hardDeleteSupplier(item.id, item.name)"
                ...
                <!-- Pagination -->
                <div v-if="suppliers.length > 0" class="flex justify-between items-center mt-4">
...
const goToAllSuppliers = () => {
    router.visit('/suppliers/all')
}

const goToDetail = (id) => {
    localStorage.setItem('supplierReferrer', '/suppliers/restore')
...
const restoreSupplier = async (id) => {
    try {
        loading.value = true
        await axios.patch(`/api/suppliers/${id}/restore`)
        ...
        let errorMessage = 'Failed to restore supplier. Please check the data or re-login.';
        let useHtml = false;
...
const hardDeleteSupplier = async (id, supplierName) => {
    const result = await Swal.fire({
        title: 'Permanently Delete Supplier',
        html: `<div class="text-left">
            <p class="mb-2">Are you sure you want to <strong>permanently delete</strong> <strong>${supplierName}</strong>?</p>
    ...
    if(result.isConfirmed){
        try {
            loading.value = true
            await axios.delete(`/api/suppliers/${id}/force`)

            Swal.fire({
                title: 'Permanently Deleted!',
                text: `${supplierName} has been permanently deleted.`,
                icon: 'success',
                confirmButtonText: 'OK',
                customClass: {
                    confirmButton: 'bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-md focus:outline-none transition-colors duration-200'
            ...
            let errorMessage = 'Failed to hard delete supplier. Please check the data or re-login.';
            let useHtml = false;
...
const fetchSuppliers = async () => {
    try {
        ...
        const response = await axios.get('/api/suppliers/trashed', {params})
