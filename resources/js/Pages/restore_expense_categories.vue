        ...
        <Sidebar :auth="auth">
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        >
                            ...
                            <p class="text-lp-green font-medium">Back to Categories</p>
                        </button>
                    </div>
                </div>
                ...
                <div v-else-if="categories.length === 0" class="text-center py-10">
                    ...
                    <h3 class="text-lg font-medium text-gray-700 mb-2">No Deleted Expense Categories</h3>
                    <p class="text-gray-500">There are no deleted expense categories to restore.</p>
                </div>
                        ...
                        <tr v-for="(item,index) in categories" :key="item.id" class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex gap-1 justify-end">
                                    ...
                                    <!-- Button Hard Delete -->
                                    <button
                                        @click="hardDeleteCategory(item.id, item.name)"
                ...
                <!-- Pagination -->
                <div v-if="categories.length > 0" class="flex justify-between items-center mt-4">
...
const restoreFilterState = () => {
    const savedState = localStorage.getItem('restoreCategoryExpenseFilterState')
    if (savedState) {
        try {
...
const goToAllCategories = () => {
    router.visit('/expense-categories/all')
}

const hardDeleteCategory = async (id, categoryName) => {
    const result = await Swal.fire({
        title: 'Permanently Delete Category',
        html: `<div class="text-left">
            <p class="mb-2">Are you sure you want to <strong>permanently delete</strong> <strong>${categoryName}</strong>?</p>
            <p class="text-red-600 font-bold">Warning: This action cannot be undone!</p>
            <p>The category will be permanently removed from the system and cannot be recovered.</p>
        </div>`,
        icon: 'warning',
    ...
    if (result.isConfirmed) {
        try {
            loading.value = true
            await axios.delete(`/api/expense-categories/${id}/force`)

            Swal.fire({
                title: 'Permanently Deleted!',
                text: `${categoryName} has been permanently deleted.`,
                icon: 'success',
            ...
            let errorHtml = '<p>Failed to hard delete category.</p>';
...
const restoreCategory = async (id) => {
    try {
        loading.value = true
        await axios.patch(`/api/expense-categories/${id}/restore`)
        ...
        const errors = error.response?.data?.errors?.message;
        let errorHtml = '<p>Failed to restore category.</p>';
...
const fetchCategories = async () => {
    try {
        loading.value = true
        const response = await axios.get('/api/expense-categories/trashed', {
            params: {
