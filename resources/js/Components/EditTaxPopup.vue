    ...
    >
        ...
        <div class="bg-white w-[750px] h-full flex flex-col">
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                <h3 class="font-semibold text-lg text-gray-900">EDIT TAX</h3>
            ...
            <div class="flex-1 overflow-auto px-6 py-4 space-y-4 text-gray-800">
                ...
                <template v-else>
                    <div>
                        <label class="block font-medium text-sm mb-1">Tax Name</label>
                        <input
                    ...
                    <div>
                        <label class="block font-medium text-sm mb-1">Tax Rate (%)</label>
                        <input
...
const fetchTax = async () => {
    try {
        ...
        const response = await axios.get(`/api/taxes/${props.taxId}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
        taxForm.value = response.data.data;
    } catch (error) {
...
const updateTax = async () => {
    try {
        ...
        await axios.patch(`/api/taxes/${props.taxId}`, taxForm.value, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
        ...
        let errorMessage = 'Failed to update tax. Please check the data or re-login.'
        let useHtml = false;
...
watch(() => props.taxId, async (newId) => {
    if (newId && props.show) {
        await fetchTax(newId);
    }
}, { immediate: true });

watch(() => props.show, async (newShow) => {
    if (newShow && props.taxId) {
        await fetchTax(props.taxId);
    }
});
